...
$APP.pathJoin = function() {
    var args = [].slice.call(arguments);
    if (args.length === 1) {
        return args[0];
    } else {
        var path = args.join('/');
        path = path.replace(/\/{2,3}/g, '/');
        return path;
    }
};

$APP.getResource = function(res) {
    return $APP.pathJoin($APP.applicationRoot, $APP.resources[res], '/');
};

$APP.asResource = function(resource,request) {
    return $APP.pathJoin($APP.getResource(resource), request);
};
...
var loadApplication = function() {
    ...
    $.ajax({
        ...
        data:       JSON.stringify({id:'arunjitsingh', password:'arunjitsingh'}),
